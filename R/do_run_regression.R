...
# check names and class
  ...
  df <- df %>% 
    tibble::rowid_to_column("window_id") %>% 
    add_rolling_observations(n = n)  %>% 
    tibble::as_tibble()
  
  # column used as index
  window_id <- df %>% 
    pull(window_id)
  
  # create rolling model objects
  ...
  suppressWarnings(
    ...
    model <- slider::slide_index(
      df, 
      window_id, 
      #  ~lmodel2::lmodel2(formula = formula, data = .x, range.y = "interval", range.x = "interval", nperm = 99),
      ~possible_lmodel2(formula = formula, data = .x, range.y = "interval", range.x = "interval", nperm = 99),
      .before = n - 1, 
      .complete = complete
    )
  ...
  df_lengths <- df_model_stats %>% 
    ...
    mutate(observations = purrr::map(observations, ~select(., -window_id))) %>% 
